...
from openedx.core.djangoapps.user_api.accounts.signals import UserRetireMailingsSignal
...
@receiver(UserRetireMailingsSignal)
def force_unsubscribe_all(sender, **kwargs):  # pylint: disable=unused-argument
    ...
    if not sailthru_response.is_ok():
        ...
        error_msg = "Error attempting to opt-out user from Sailthru: %s" % error.get_message()
        log.error(error_msg)
        raise Exception(error_msg)
